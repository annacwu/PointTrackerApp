...
import { TAG_GREEN, TAG_BLUE, TAG_PURPLE, BORDER_LIGHT_GREY } from "../utils/colors";
    ...
    return (
        <View style={styles.container}>
            <ScrollView contentContainerStyle={styles.scroll} horizontal={true} showsHorizontalScrollIndicator={false}>
                {colors.map(color => 
                    <TouchableOpacity key={color} style={[styles.circle, { backgroundColor: color }, ...(itemColor === color ? [styles.selectedCircle] : [])]} onPress={() => onSelected(color)}/>
                )}
            </ScrollView>
        </View>
    );
